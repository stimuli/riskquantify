...
const char * const rq_asset_type_id_commodity = "Commodity";


void
rq_asset_commodity_freefunc(void *d)
{
    struct rq_asset_commodity *c = (struct rq_asset_commodity *)d;
...
{
    struct rq_asset_commodity *c = (struct rq_asset_commodity *)calloc(
        1, 
        sizeof(struct rq_asset_commodity)
        );
    rq_asset_t asset = _rq_asset_alloc(
        rq_asset_type_id_commodity, 
        asset_id,
        c,
        rq_asset_commodity_freefunc
        );
...
{
    return !strcmp(rq_asset_get_asset_type_id(asset), rq_asset_type_id_commodity);
}

RQ_EXPORT const char *
rq_asset_commodity_get_long_name(rq_asset_t asset)
{
    struct rq_asset_commodity *c = (struct rq_asset_commodity *)_rq_asset_get_asset_type_data(asset);
    return c->long_name;
}
